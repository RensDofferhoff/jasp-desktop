...
#      included by construction) in the D11 storage-token vocabulary (the base's
#      own field names), lets the implied build run;
#   4. reads the blob (read_feather) and cross-checks the D11 identity: blob fields
#      are <token>_<type> — exactly the aliases the REAL codec (pulled from
#      runner_jaspbase.R) derives from the decoded displays — so the slice-B read
#      is an identity, plus the {token: display} map against the base's metadata;
...
# The alias codec + storage tokens, straight from the runner's parse tree
# (walk_test.R pattern — the runner file cannot be source()d: it dials the
# orchestrator at top level).
want <- c("ALIAS_PREFIX", "ALIAS_TYPES", "alias_encode", "token_of", "token_decode")
...
run_battery <- function(tag, dataset_id, revision, cols) {
  ...
  # ── the blob, as slice B reads it (D11: fields are <token>_<type> — the R aliases) ──
  blob <- refs[[1]]$path
  tbl <- arrow::read_feather(blob, as_data_frame = FALSE)
  meta <- fromJSON(tbl$schema$metadata[["jasp:view"]], simplifyVector = FALSE)
  df <- as.data.frame(tbl)
  # The base's display map (footer metadata) — the decode truth for the cross-checks.
  base_tbl <- arrow::read_feather(base, as_data_frame = FALSE)
  bdisp <- setNames(vapply(base_tbl$schema$names, function(i) {
    m <- base_tbl$schema$GetFieldByName(i)$metadata[["jasp:display_name"]]
    if (is.null(m) || !nzchar(m)) i else m
  }, ""), base_tbl$schema$names)

  expected_fields <- c(vapply(spec_cols, function(c) sprintf("%s_%s", c$name, c$as), ""),
                       "__base_row")
  ...
  check(sprintf("[%s] token_map == {token: display} (D2/D11 cross-check)", tag),
        length(meta$token_map) == length(unique(vapply(spec_cols, function(c) c$name, ""))) &&
        all(vapply(spec_cols, function(c)
          identical(unname(meta$token_map[[c$name]]),
                    unname(bdisp[[c$name]])),      # the base's own metadata
          logical(1L))))
  check(sprintf("[%s] view_id == hash-named blob (content address)", tag),
        grepl("^[0-9a-f]{64}$", meta$view_id))

  # The D11 identity: each blob field IS the alias the codec derives from the decoded
  # display — the slice-B rename is an identity, and the codec agrees with the base's
  # metadata (a mismatch would mean a non-deterministic build, which AV8 forbids).
  ...
  real <- vapply(token, function(t) token_decode(t) %||% stop("not a token: ", t), "")
  # NOTE: compare by VALUE (==) — identical() is encoding-sensitive and the codec
  # output is UTF-8-marked while Arrow's strings arrive unmarked.
  d11_identity <-
    all(mapply(alias_encode, real, ty, USE.NAMES = FALSE) == fields) &&
    all(real == unname(bdisp[token]))
  if (!check(sprintf("[%s] D11 identity: fields == codec(display, type)", tag),
             d11_identity)) {
    cmp <- data.frame(field = fields, token = token, type = ty,
                      decoded = real, base_meta = unname(bdisp[token]),
                      codec_alias = unname(mapply(alias_encode, real, ty, USE.NAMES = FALSE)))
    print(cmp)
  }
  ...
  for (i in seq_along(spec_cols)) {
    ...
    # `ordered` flag (probe-pinned), so a builder-side flag error shows up here as a
    # class mismatch — never papered over, no introspection needed.
...
R0 <- run_battery("rchar-r0", d3$dataset_id, d3$revision, c(token_of("v"), token_of("mix")))

# NaN vs NA in the BASE itself (the trichotomy the scale/nominal casts pin).
v_base <- arrow::read_feather(R0$base)[[token_of("v")]]
...
v_nom <- paste0(token_of("v"), "_nominal")    # D11 blob fields: <token>_<type>
v_scl <- paste0(token_of("v"), "_scale")
act_levels <- levels(R0$df[[v_nom]])
check("[rchar] %.15g grouping: merge pairs collapse (0.3==0.1+0.2; 16-digit ints)",
      ...
      !identical(levels(R0$df[[v_nom]]) %in% "0.3", logical(0)) &&
...
check("[rchar] f64->nominal row values == jasp_level_string(v) (non-finite -> NA)",
      identical(as.character(R0$df[[v_nom]]), want_vals))
# scale keeps NaN as NaN (never collapsed to NA).
check("[rchar] f64->scale: NaN row survives as NaN",
      sum(is.nan(R0$df[[v_scl]])) == 1L &&
      identical(which(is.nan(R0$df[[v_scl]])), which(is.nan(v_base))))

# ── battery 4: label overlay via schema_change (debug/group, rev 1) ───────────
cat("\n== label overlay: schema_change labels on debug/group ==\n")
# The edit lane's `name` identity is the storage token (debug_cols are tokens);
# schema_change additionally resolves display-named entries during migration.
entries <- lapply(debug_cols, function(nm)
  if (identical(token_decode(nm), "group")) list(name = nm, labels = list(A = "Alpha A")) else list(name = nm))
...
check("[labels] group nominal levels carry the overlay",
      identical(levels(A1$df[[paste0(token_of("group"), "_nominal")]]), c("Alpha A", "B")))
...
entries <- lapply(torture_cols, function(nm)
  if (identical(token_decode(nm), "groep")) list(name = nm, levels = c("B", "A")) else list(name = nm))
...
check("[remap] groep nominal level ORDER preserved (B before A — never re-sorted)",
      identical(levels(T1$df[[paste0(token_of("groep"), "_nominal")]]), c("B", "A")))
