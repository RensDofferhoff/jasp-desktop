...
Json::Value AnalysisForm::_controlOptionMeta(JASPControl* ctrl, bool includeDescriptions) const
{
	...
	case JASPControl::ControlType::VariablesListView:
	{
		...
		if (listCtrl)
		{
			...
			if (provider)
			{
				const QStringList names = provider->provideInfo(varInfoType::VariableNames).toStringList();

				Json::Value vars(Json::objectValue);
				for (const QString & name : names)
					vars[name.toStdString()] = columnTypeToString(columnType(provider->provideInfo(varInfoType::VariableType, name).toInt()));
