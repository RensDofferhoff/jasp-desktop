...
# + runner_jaspbase.R + R frontend. The frontend opens test_data/debug.csv by submitting a
# data_open WORK (→ terminal result with the dataset_id), then a real
...
echo "=== starting data-runner (csv lane) ==="
./orchestrator/target/debug/jasp-data-runner > /tmp/data_ttest.log 2>&1 &
DATA_PID=$!
...
echo "===== data-runner log ====="; cat /tmp/data_ttest.log
