...
void DatabaseInterface::dataSetBatchedValuesUpdate(DataSet * data)
{
	...
	bindParametersType bindParamStore = [&](sqlite3_stmt * stmt)
	{
		size_t i=1;
		for(Column * col : data->columns())
			if(col->type() == columnType::scale)	sqlite3_bind_double(stmt,	i++, col->dbls()[rowOutside]);
...
void DatabaseInterface::dataSetBatchedValuesLoad(DataSet *data)
{
	...
	std::function<void(size_t, sqlite3_stmt *stmt)> processRow = [&](size_t row, sqlite3_stmt *stmt)
	{
		...
		for(size_t colI=0; colI<data->columns().size(); colI++)
		{
			...
			{
				if(col->type() == columnType::scale)	col->setValue(row, sqlite3_column_double(	stmt, colI), false);
...
void DatabaseInterface::columnSetValues(int columnId, const doublevec &dbls)
{
	...
	bindParametersType bindParamStore = [&](sqlite3_stmt * stmt)
	{
		sqlite3_bind_double(stmt,	1, dbls[rowOutside]);
		sqlite3_bind_int(stmt,		2, rowOutside+1);
	};
...
void DatabaseInterface::columnGetValuesDbls(int columnId, doublevec & dbls)
{
	...
	std::function<void(size_t, sqlite3_stmt *stmt)> processRow = [&](size_t row, sqlite3_stmt *stmt)
	{
		...
		dbls[row] = sqlite3_column_double(stmt, 0);
	};
